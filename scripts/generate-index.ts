...
try {
  ...
  const header = `\
/**
 * DO NOT EDIT THIS FILE WHICH WAS GENERATED BY '${path.relative(packageDirectoryPath, ownFilename)}'.
 */`
...
async function generateIndex(type: IndexFileType, header: string) {
  ...
  const fileNames = await fs.readdir(path.resolve(ownDirname, `../src/${type}`))
  const sanitizedFileNames = fileNames.filter((fileName) => {
    return !fileName.startsWith('.gitkeep') && !fileName.endsWith('.ts')
  })
  const targetFile = path.resolve(ownDirname, `../src/${type}.ts`)
